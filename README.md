...
The input shape for the data batches immediately prior before ingestion into the embedding network is `(N, C, H, W)` where the `C` axis runs over the different one-hot encoded land cover classes and an additional channel for the DEM.<img src="banner.png" width="900">
